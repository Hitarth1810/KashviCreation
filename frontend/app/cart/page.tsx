...
function CartPage() {
	...
	const handleBuyNow = async () => {
		if (!user) {
			// If user is not logged in, redirect to login and return to cart after login
			router.push("/login?redirect=/cart");
			return;
		}

		if ((await getShippingAddress(user.id))?.length === 0) {
			// If user is logged in but has no shipping address, redirect to profile setup
			setIsAddressFormOpen(true);
			return;
		}

		// If user is logged in and has a shipping address, proceed to checkout
		clearCart();
					...
					<AnimatePresence mode='wait'>
						{cartItems.length === 0 ? (
							...
							>
								...
								<p className='text-2xl text-amber-800/70 font-medium mb-4'>
									Your cart is empty
								</p>
								<button
										...
										<div className='flex-1 space-y-3'>
											...
											<div className='flex items-center gap-4'>
												<div className='flex items-center border-2 border-amber-500 rounded-full overflow-hidden'>
													...
													<span className='w-12 text-center font-medium text-amber-800'>
														{item.quantity}
													</span>
													<button
														onClick={() => updateQuantity(item.id, 1)}
