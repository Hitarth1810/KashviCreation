      ...
      <AnimatePresence>
        {!isLoading && filteredSarees.length === 0 ? (
          <div className="col-span-full text-center text-gray-600 text-lg py-10">
            Sorry, no such sarees are available.
          </div>
        ) : (
...
function CollectionsWithSearchParams() {
  ...
  useEffect(() => {
    if (categoryQuery) {
      setFilters((prevFilters) => ({
        ...prevFilters,
        categories: [categoryQuery], // Set the selected category
      }));
    }
  }, [categoryQuery]);
...
// Main component that wraps the searchParams component in a Suspense boundary
